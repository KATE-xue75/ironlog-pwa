...
const App = {
  ...
  shareCard() {
    ...
    // CTA line
    ctx.fillStyle = '#f25f4c'; ctx.font = 'bold 28px Inter, -apple-system, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('Track free → ironlog-pwa-1ls.pages.dev', 540, 1260);

    // URL watermark
    ctx.fillStyle = 'rgba(106, 125, 142, 0.45)'; ctx.font = '17px Inter, -apple-system, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('ironlog-pwa-1ls.pages.dev', 540, 1300);

    ctx.fillStyle = '#9aacbc'; ctx.font = '20px Inter, -apple-system, sans-serif';
    ctx.textAlign = 'center';
    ctx.fillText('Generated by IronLog · No login. No ads. Just lift.', 540, 1330);

    canvas.toBlob(blob => {
      const url = URL.createObjectURL(blob);
      this._showShareModal(url, blob);
    });
  },

  // ============ SHARE MODAL ============
  _showShareModal(imageUrl, blob) {
    // Remove any existing modal
    const existing = document.getElementById('share-modal');
    if (existing) existing.remove();

    const shareUrl = 'ironlog-pwa-1ls.pages.dev';
    const hasNativeShare = !!(navigator.share && navigator.canShare);

    const overlay = document.createElement('div');
    overlay.id = 'share-modal';
    overlay.style.cssText = 'position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(15,26,44,0.85);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;box-sizing:border-box;';

    const card = document.createElement('div');
    card.style.cssText = 'background:#f8fafb;border-radius:16px;max-width:400px;width:100%;max-height:90vh;overflow-y:auto;padding:20px;box-shadow:0 20px 60px rgba(0,0,0,0.3);position:relative;';

    // Close button
    const closeBtn = document.createElement('button');
    closeBtn.innerHTML = '&times;';
    closeBtn.style.cssText = 'position:absolute;top:12px;right:12px;width:36px;height:36px;border-radius:50%;border:none;background:#e8ecf0;color:#0f1a2c;font-size:22px;cursor:pointer;display:flex;align-items:center;justify-content:center;line-height:1;z-index:1;';
    closeBtn.onclick = () => overlay.remove();
    card.appendChild(closeBtn);

    // Title
    const title = document.createElement('div');
    title.textContent = '💪 Share Your Workout';
    title.style.cssText = 'text-align:center;font-size:20px;font-weight:700;color:#0f1a2c;margin-bottom:16px;padding-right:30px;';
    card.appendChild(title);

    // Image preview
    const img = document.createElement('img');
    img.src = imageUrl;
    img.style.cssText = 'width:100%;border-radius:12px;box-shadow:0 2px 12px rgba(0,0,0,0.1);display:block;';
    ...
    copyBtn.style.cssText = 'flex:1;min-width:100px;padding:12px 16px;border-radius:10px;border:none;background:#e8ecf0;color:#0f1a2c;font-size:15px;font-weight:600;cursor:pointer;transition:background 0.15s;';
    copyBtn.onmouseenter = () => { copyBtn.style.background = '#dde3e8'; };
    copyBtn.onmouseleave = () => { copyBtn.style.background = '#e8ecf0'; };
    copyBtn.onclick = () => {
    ...
    downloadBtn.style.cssText = 'flex:1;min-width:100px;padding:12px 16px;border-radius:10px;border:none;background:#e8ecf0;color:#0f1a2c;font-size:15px;font-weight:600;cursor:pointer;transition:background 0.15s;';
    downloadBtn.onmouseenter = () => { downloadBtn.style.background = '#dde3e8'; };
    downloadBtn.onmouseleave = () => { downloadBtn.style.background = '#e8ecf0'; };
    downloadBtn.onclick = () => {
    ...
    if (hasNativeShare) {
      ...
      shareBtn.style.cssText = 'flex:1;min-width:100px;padding:12px 16px;border-radius:10px;border:none;background:#f25f4c;color:#fff;font-size:15px;font-weight:600;cursor:pointer;transition:background 0.15s;';
      shareBtn.onmouseenter = () => { shareBtn.style.background = '#e04d3b'; };
      shareBtn.onmouseleave = () => { shareBtn.style.background = '#f25f4c'; };
      shareBtn.onclick = () => {
        const file = new File([blob], 'ironlog-workout.png', { type: 'image/png' });
        navigator.share({ files: [file], title: 'My IronLog Workout' }).catch(() => {});
      };
